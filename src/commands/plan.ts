...
export const planCommand = new Command('plan')
  ...
  .action(async (sourcePath?: string) => {
    ...
    if (!sourcePath) {
      const result = await selectFileInteractively('plan');
      if (!result) {
        process.exit(0);
      }
    ...
    await executePlan(sourcePath);
  });

/**
 * Plan 프롬프트 생성 실행
 */
const executePlan = async (sourcePath: string): Promise<void> => {
  try {
    ...
    logger.hint(
      `ATDD 시나리오, 프로젝트 설정, 소스 코드(${sourcePath})와 Test Routing 프롬프트가 결합되어 AI에게 붙여넣을 준비가 되었습니다.`,
    );
  } catch (error: unknown) {
    // 에러 핸들링 세분화
