...
**통과 기준**: 출력에 "error" 문자열이 없어야 함 (0 errors)

**에러 분류 및 처리**:

| 분류 | 예시 | 조치 |
|------|------|------|
| 코드 에러 | `property does not exist`, `Type 'X' is not assignable` | **즉시 수정 필수** |
| 환경 에러 | `Cannot find module` (설정 문제) | 설정 수정 후 재검증 |
| 무시 가능 | 외부 라이브러리 타입 충돌 (skipLibCheck로 해결) | 사유 명시 후 진행 |

> ⚠️ **금지**: 에러를 "환경 관련"으로 모호하게 분류하지 마세요.
> 에러 코드(TS2353 등)와 정확한 원인을 보고해야 합니다.

**실패 시 처리**:
1. 에러 메시지 분석 (에러 코드, 파일명, 라인 번호 확인)
2. 타입 오류 수정 (import 경로, 타입 불일치, 잘못된 필드명 등)
3. 재검사하여 **0 errors** 확인
