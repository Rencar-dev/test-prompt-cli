<!-- Source: atdd-test-routing-prompt.md -->
# 🧭 ATDD Test Routing Prompt
...
이 프롬프트의 출력이 성공적이라면:
- [ ] ATDD의 **모든 시나리오**가 분류 테이블에 포함됨 (누락 없음)
- [ ] 각 시나리오에 적절한 프롬프트(`business-logic-test` / `ui-test-implementation`)가 매핑됨
- [ ] `[E2E]` 시나리오가 `router.push/replace` 호출 검증으로 **번역**됨
- [ ] MSW 핸들러 **코드가 직접 작성되지 않음** (Planning만)
- [ ] 조건부 분기("또는", "~이면")가 있는 시나리오는 **분기별로 명시**됨

#### How to Validate (검증 방법)
1. ATDD 시나리오 ID 목록과 Plan 테이블의 ID 목록 비교 (누락 확인)
2. `[E2E]` 태그 시나리오의 Note에 "router 호출 검증" 언급 여부 확인
3. 출력에 `http.get`, `http.post` 등 MSW 핸들러 코드 포함 여부 검색
4. "또는" 키워드가 포함된 ATDD 시나리오의 분기 명시 여부 확인
